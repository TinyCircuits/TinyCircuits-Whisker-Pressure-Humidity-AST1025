...
void loop() {
  ...
  if (bme.begin()) {
    // Delay because sensor reading immediately after begin() produces garbage output
    delay(100); 
    printValues(); // Print to serial monitor
  }  
  else{
    SerialMonitorInterface.println("Could not find a valid BME280 sensor on port: "); 
    SerialMonitorInterface.println(portNum);
  }
...
void printValues() {
  SerialMonitorInterface.print("Port number = ");
  SerialMonitorInterface.println(portNum);
